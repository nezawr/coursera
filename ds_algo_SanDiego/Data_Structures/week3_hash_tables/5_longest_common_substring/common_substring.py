...
class Solver:
	def __init__(self, s, t):
		self.s = s
		self.t = t
		self._prime1, self._prime2, self._prime3 = 10**9 + 7, 10**9 + 9, 13591054291343
		...
		self.H3s = precompute_hashes(s, self._prime3, self._multiplier)
		self.H1t = precompute_hashes(t, self._prime1, self._multiplier)
		self.H2t = precompute_hashes(t, self._prime2, self._multiplier)
		self.H3t = precompute_hashes(t, self._prime3, self._multiplier)
...
def AllHashValues(solver, start, length, bool):
	if bool:
		...
		val3 = HashValue(solver.H3s, solver._prime3, solver._multiplier, start, length)
	else:
		...
		val3 = HashValue(solver.H3t, solver._prime3, solver._multiplier, start, length)
...
def hash_checker(t_hash_tup, s_values):
	...
	val3 = t_hash_tup[2] in s_values[2]
	return val1 and val2 and val3

def get_hash_values(s, length, solver):
	hash1, hash2, hash3 = {}, {}, {}
	for i in range(0, len(s) - length + 1):
		val1, val2, val3 = AllHashValues(solver, i, length, True)
		hash1[val1], hash2[val2], hash3[val3] = i, i ,i
